...
def grade(waypoints, solution, visualize=False):
    ...
    results = {'d_thresh': 3,  # distance to waypoints
        'v_thresh': 3,  # speed difference at waypoints
        'pass_percentage': 50,  # percentage of correct waypoints required
        'solution': solution, 'waypoints': waypoints, 'dists2': np.zeros((N, 1)), 'vels': np.zeros((N, 1)),
        'inds': np.zeros((N, 1), dtype='int32'), 'pass_dist': True, 'pass_vel': True}
    ...
    if percent_correct < results['pass_percentage']:
        print(
            'Assessment failed, only {0:.2f}% of waypoints completed. {1:.2f}% required to pass'.format(percent_correct,
                                                                                                        results[
                                                                                                            'pass_percentage']))
    else:
